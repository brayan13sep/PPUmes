...
-- -----------------------------------------------------
-- Table `hospital`.`RFID_log`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `hospital`.`RFID_log` ;

CREATE TABLE IF NOT EXISTS `hospital`.`RFID_log` (
  `ID` INT NOT NULL AUTO_INCREMENT,
  ...
  `Usuario_ID` INT NOT NULL,
  PRIMARY KEY (`ID`, `log_fecha`, `Usuario_ID`),
  INDEX `fk_RFID_log_Usuario1_idx` (`Usuario_ID` ASC),
  CONSTRAINT `fk_RFID_log_Usuario1`
    FOREIGN KEY (`Usuario_ID`)
    REFERENCES `hospital`.`Usuario` (`ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `hospital`.`RFID_log_madre_neonato` (
  ...
  `log_fecha` TIMESTAMP NOT NULL,
  `area` VARCHAR(255) NOT NULL,
  `rfid_valor` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`ID`, `log_fecha`, `rfid_valor`),
  INDEX `fk_RFID_log_madre_neonato_RFID_madre_neonato1_idx` (`rfid_valor` ASC),
  CONSTRAINT `fk_RFID_log_madre_neonato_RFID_madre_neonato1`
    FOREIGN KEY (`rfid_valor`)
    REFERENCES `hospital`.`RFID_madre_neonato` (`valor`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
